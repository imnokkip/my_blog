...
def music_list(request):
    tracks = MusicTrack.objects.filter(is_published=True).order_by('-created_at')
    return render(request, 'main/music.html', {'tracks': tracks})

def music_detail(request, track_id):
    track = get_object_or_404(MusicTrack, pk=track_id, is_published=True)
    return render(request, 'main/music_detail.html', {'track': track})